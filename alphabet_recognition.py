            ...
            for sequence in range(no_sequences):
                ...
                for frame_num in range(sequence_length):
                    ...
                    if frame_num == 0:
                        ...
                        cv2.putText(image, 'Collecting frames for {}'.format(action),
                                    (20,20),
                                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
                        cv2.putText(image, 'Video Number {}'.format( sequence),
                                    (20,50),
                                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
                        # Show to screen
...
def preprocess_data_and_train_model():
    ...
    log_dir = os.path.join('Alphabets_Logs')
...
def test_real_time():
    ...
    model = keras.models.load_model('alphabets.h5')
    sequence = []
    sentence = []
    prediction  =""
    cap = cv2.VideoCapture(0)
        ...
        while cap.isOpened():
            ...
            if results.face_landmarks and results.pose_landmarks and (results.left_hand_landmarks or results.right_hand_landmarks):
                ...
                if prediction != "":
                    ...
                    image=cv2.putText(image,f"{''.join(prediction)}",(30,100),cv2.FONT_HERSHEY_SIMPLEX,1,(255,233,44),2,cv2.LINE_AA)
                cv2.imshow('OpenCV Feed', image)
                ...
                if prediction!="":
                    if prediction not in sentence:
                        sentence.append(prediction)
                    print(sentence)
                    image=cv2.putText(image,f"Detected: {''.join(sentence)}",(30,100),cv2.FONT_HERSHEY_SIMPLEX,1,(255,233,44),2,cv2.LINE_AA)
