...
def preprocess_data_and_train_model():
    ...
    log_dir = os.path.join('Numbers_Logs')
    ...
    # Save the model.
    with open('numbers.tflite', 'wb') as f:
...
def test_real_time():
    ...
    with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
        while cap.isOpened():
            ret, frame = cap.read()
            image, results = mediapipe_detection(frame, holistic)
            if results.face_landmarks and results.pose_landmarks and (
                ...
                if prediction != "":
                    ...
                    image = cv2.putText(image, f"Detecting: {prediction}", (30, 100), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                        (255, 233, 44), 2, cv2.LINE_AA)
                cv2.imshow('OpenCV Feed', image)
                ...
                if prediction != "":
                    if prediction not in sentence:
                        sentence.append(prediction)
                    print(sentence)
                    image = cv2.putText(image, f"Detected: {''.join(sentence)}", (30, 100), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                        (255, 233, 44), 2, cv2.LINE_AA)
                image = cv2.putText(image, "Unknown Gesture", (30, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 233, 44), 3,
                                    cv2.LINE_AA)
                cv2.imshow('OpenCV Feed', image)
...
# collect_keypoints_from_images(f)
